...
- Initialize from the first accepted viewport raster: B = 0.5 times inverse luminance, A = 1 - B. Do not use scattered seeds. Wait for this snapshot before starting the simulation; later captures only provide weak continuous B input. If initial capture fails, keep the page static.
